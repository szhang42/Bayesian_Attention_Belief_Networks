        ...
        if self.HP.att_type == 'gamma_att':
            ...
            if self.training:
                ...
                if self.HP.model_type =='whai':
                    ...
                    # lambda_weibull = ((torch.tensor(1e-4).type(torch.float32)) * (torch.log(1 + torch.exp(torch.matmul(self.w_2, hidden) + self.b_2) + eps))) + (torch.exp(scores)/torch.exp(torch.lgamma(1 + 1.0 / k_weibull).type_as(u_weibull)))
...
class MCA_ED(nn.Module):
    ...
    def forward(self, x, y, x_mask, y_mask, att_map_last_SA, att_map_last_SGA_1, att_map_last_SGA_2, h_SA, h_SGA_1, h_SGA_2, samp_wei_SA, samp_wei_SGA_1, samp_wei_SGA_2):
        ...
        if self.HP.model_type == 'whai':
            ...
            i = -1
            for enc in self.enc_list:
                temp_x, hidden_mid_SA, temp_sample_mid_SA = enc(x, x_mask, att_map_last, hidden, sample_weibull, laysa)
                hidden=hidden_mid_SA
                temp_hidden_mid_SA.append(hidden_mid_SA)
            for enc in self.enc_list:
                x, hidden_mid_SA, sample_mid_SA = enc(x, x_mask, att_map_last, temp_hidden_mid_SA[-1], sample_weibull, laysa)
            ...
            laysga = 0
            temp_hidden_mid_SGA_1=[]
            temp_hidden_mid_SGA_2=[]
            lst= -1
            for dec in self.dec_list:
                temp_y, hidden_mid_SGA_1, hidden_mid_SGA_2, temp_sample_mid_SGA_1, temp_sample_mid_SGA_2  = dec(y, x, y_mask, x_mask, att_map_last_SGA_final_1,
                ...
                temp_hidden_mid_SGA_1.append(hidden_mid_SGA_1)
                temp_hidden_mid_SGA_2.append(hidden_mid_SGA_2)

            for dec in self.dec_list:
                y, hidden_mid_SGA_1, hidden_mid_SGA_2, sample_mid_SGA_1, sample_mid_SGA_2  = dec(y, x, y_mask, x_mask, att_map_last_SGA_final_1,
                                                                  att_map_last_SGA_final_2, temp_hidden_mid_SGA_1[-1], temp_hidden_mid_SGA_2[-1], sample_weibull_SGA_1, sample_weibull_SGA_2, laysga)
